...
class Contacto {
    ...
    // Método para generar la tabla de contactos
    public static function generarTablaContactos($rolUsuario) {
        ...
        if ($arrContactos) {
            foreach ($arrContactos as $contacto) {
                $html .= "<tr>
                    ...
                    <td>" . htmlspecialchars($contacto->getDireccion()) . "</td>
                    <td>" . htmlspecialchars($contacto->getTelefono()) . "</td>
                ...
                if ($rolUsuario == "1") {
                    $html .= "
                        <form method='post' action='abcContacto.php' class='formEliminar'>
                            <input type='hidden' name='txtClave' value='" . $contacto->getId() . "'>
                    ...
                    $html .= "Solo Visualizar";
                }

                $html .= "</td></tr>";
            }
        } else {
            $html .= "<tr><td colspan='5'>No hay contactos registrados</td></tr>";
        }

        $html .= "</table>";

        // Si el rol es administrador, incluye el botón para agregar un nuevo contacto
        if ($rolUsuario == "1") {
            $html .= "<input type='submit' value='Agregar nuevo contacto' onclick=\"txtClave.value='-1'; txtOpe.value='a'\">";
        }
